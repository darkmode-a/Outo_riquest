...
def get_emoji_id(plan_emoji):
    if plan_emoji in PREMIUM_EMOJI_MAP:
        return random.choice(PREMIUM_EMOJI_MAP[plan_emoji])
    return None

# ═══════ COLORED BUTTON (PTB STYLE) ═══════
def colored_btn(text, callback=None, url=None, color="primary"):
    if url:
    ...
    if contents:
        for item in contents:
            try:
                markup = None
                if item.get("buttons"):
                    markup = build_keyboard(item["buttons"])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        ...
        text = f"🏆 <b>ALL-IN-ONE BOT</b>\n\n👑 Admin: {user.first_name}\n📥 Join Accept: {join_status}"
        keyboard = [
            [colored_btn("START", callback="join_on", color="success"), colored_btn("OFF", callback="join_off", color="danger")],
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "join_on":
        ...
        save_data(data)
        await query.message.reply_text("🟢 Join Accept ON!")
        return
    elif cmd == "join_off":
        data["join_enabled"] = False
        save_data(data)
        await query.message.reply_text("🔴 Join Accept OFF!")
        return
        ...
        await query.message.reply_text("📝 Welcome text bhejo:\n\n/cancel")
    elif cmd == "add_file":
        ...
        await query.message.reply_text("📌 Number bhejo (0=unpin):")
    elif cmd == "clear":
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "adding_text":
        ...
        if added > 0:
            save_data(data)
            await update.message.reply_text(f"✅ {added} buttons added!")
        user_states.pop(uid, None)
    elif state == "pin_select":
        try:
